...
// ── Skeleton primitives ───────────────────────────────────────
function SkeletonBox({ width = "100%", height = 16, radius = 6, style: extra = {}, darkMode }) {
...
function KpiCardSkeleton({ darkMode, index = 0 }) {
  ...
  const accents = ["#1877F2", "#36a420", "#e02020", "#f59e0b", "#0891b2", "#9333ea"];
  ...
  return (
    <div style={{
      ...
      animation: "analytics-fade-slide-up 0.4s ease both, analytics-pulse-soft 2s ease-in-out infinite",
...
function SectionCardSkeleton({ height = 220, darkMode, children, delay = 0 }) {
  return (
    <div style={{
      ...
      overflow: "hidden",
      boxShadow: neuShadow(darkMode),
      animation: "analytics-fade-slide-up 0.45s ease both, analytics-pulse-soft 2.2s ease-in-out infinite",
      ...
      <div style={{ padding: "16px 18px" }}>{children || <SkeletonBox width="100%" height={height} radius={8} darkMode={darkMode} />}</div>
    </div>
  );
}
...
function FadeSlideIn({ children, delay = 0, style: extra = {} }) {
  return (<div style={{ animation: `analytics-fade-slide-up 0.45s ease both`, animationDelay: `${delay}ms`, ...extra }}>{children}</div>);
}
...
function MiniBar({ value, max, color, darkMode }) {
  ...
  return (
    <div style={{ flex: 1, height: 8, borderRadius: 99, boxShadow: neuInputShadow(darkMode), overflow: "hidden", background: neuCardBg(darkMode) }}>
...
function TrendChart({ data, ui, darkMode }) {
  ...
  if (!data || data.length === 0) {
    return (<div style={{ height: 200, display: "flex", alignItems: "center", justifyContent: "center", color: ui.textMuted, fontSize: "0.84rem" }}>No trend data available</div>);
  }
  const W = 680, H = 200;
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block", overflow: "visible" }}>
        {yticks.map((t, i) => (
          <g key={i}>
            <line x1={PAD.left} y1={toY(t)} x2={W - PAD.right} y2={toY(t)} stroke={darkMode ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)"} strokeWidth={i === 0 ? 1.5 : 0.75} strokeDasharray={i === 0 ? "none" : "3 4"} />
            <text x={PAD.left - 7} y={toY(t) + 4} textAnchor="end" fill={ui.textMuted} fontSize="9" fontWeight="500">{t}</text>
          </g>
        ))}
        {series.map(s => (<path key={s.key} d={makePath(s)} fill="none" stroke={s.color} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />))}
        {data.map((d, i) => (
          <g key={i}>
            <rect x={toX(i) - cW / data.length / 2} y={PAD.top} width={cW / data.length} height={cH} fill="transparent" onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} />
            {hov === i && (<>
              <line x1={toX(i)} y1={PAD.top} x2={toX(i)} y2={PAD.top + cH} stroke={ui.textMuted} strokeWidth={1} strokeDasharray="3 3" opacity={0.5} />
              {series.map(s => (<circle key={s.key} cx={toX(i)} cy={toY(d[s.key] ?? 0)} r={4} fill={s.color} stroke={neuCardBg(darkMode)} strokeWidth={2} />))}
              <g>
                <rect x={toX(i) > W * 0.65 ? toX(i) - 130 : toX(i) + 10} y={PAD.top + 4} width={120} height={18 + series.length * 18 + 8} rx={10} fill={neuCardBg(darkMode)} stroke="none" filter="url(#neuTooltip)" />
                <text x={toX(i) > W * 0.65 ? toX(i) - 70 : toX(i) + 70} y={PAD.top + 17} textAnchor="middle" fill={FB} fontSize={9.5} fontWeight={700}>{d.label}</text>
                {series.map((s, si) => (
                  <g key={s.key}>
                    ...
                    <text x={toX(i) > W * 0.65 ? toX(i) - 18 : toX(i) + 122} y={PAD.top + 32 + si * 18} textAnchor="end" fill={s.color} fontSize={9} fontWeight={700}>{d[s.key] ?? 0}</text>
                  </g>
                ))}
              </g>
            </>)}
...
function DonutChart({ data, ui, darkMode }) {
  ...
  let sa = -Math.PI / 2;
  const slices = data.map((d, i) => { const angle = total === 0 ? 0 : (d.value / total) * 2 * Math.PI; const s = { ...d, startAngle: sa, endAngle: sa + angle, color: colors[i] }; sa += angle; return s; });
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
      <div style={{ position: "relative" }}>
        <svg viewBox="0 0 180 180" style={{ width: 160, height: 160, flexShrink: 0, overflow: "visible" }}>
          <circle cx={cx} cy={cy} r={(r + ri) / 2} fill="none" stroke={darkMode ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)"} strokeWidth={r - ri + 2} />
          {slices.map((s, i) => { const isAct = i === active; const or = isAct ? r + 5 : r, ir2 = isAct ? ri - 2 : ri; return (<path key={i} d={arc(s.startAngle, s.endAngle, or, ir2)} fill={s.color} opacity={isAct ? 1 : 0.7} onMouseEnter={() => setActive(i)} style={{ cursor: "pointer", transition: "all 0.2s", filter: isAct ? `drop-shadow(0 2px 6px ${s.color}60)` : "none" }} />); })}
        ...
        {slices.map((s, i) => {
          ...
          return (
            <div key={s.name} onMouseEnter={() => setActive(i)} style={{ cursor: "pointer" }}>
              ...
              <div style={{ height: 5, borderRadius: 99, boxShadow: neuInputShadow(darkMode), background: neuCardBg(darkMode), overflow: "hidden" }}>
...
// ── Neumorphic KPI Card ──────────────────────────────────────
function KpiCard({ icon, label, value, color, sub, darkMode, ui, animDelay = 0 }) {
    ...
    <FadeSlideIn delay={animDelay}>
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ width: 32, height: 32, borderRadius: 10, background: neuCardBg(darkMode), boxShadow: neuShadow(darkMode), display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1rem" }}>{icon}</div>
        </div>
        <div style={{ fontSize: "1.9rem", fontWeight: 800, color, lineHeight: 1, letterSpacing: "-0.03em" }}>{animated}</div>
...
function SectionCard({ title, subtitle, icon, children, ui, darkMode, action }) {
  return (
    <div style={{ background: neuCardBg(darkMode), borderRadius: 16, overflow: "hidden", boxShadow: neuShadow(darkMode) }}>
      <div style={{ padding: "14px 18px 12px", borderBottom: `1px solid ${darkMode ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.06)"}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <p style={{ margin: 0, fontSize: "0.88rem", fontWeight: 700, color: ui.textPrimary }}>{icon && <span style={{ marginRight: 6 }}>{icon}</span>}{title}</p>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <SkeletonBox width={200} height={34} radius={12} darkMode={darkMode} style={{ boxShadow: neuShadow(darkMode, false) }} />
          <SkeletonBox width={90} height={34} radius={12} darkMode={darkMode} style={{ boxShadow: neuShadow(darkMode) }} />
          <SkeletonBox width={110} height={34} radius={12} darkMode={darkMode} style={{ boxShadow: neuShadow(darkMode) }} />
          <SkeletonBox width={88} height={34} radius={12} darkMode={darkMode} style={{ boxShadow: neuShadow(darkMode) }} />
        </div>
      </div>

      {/* KPI cards with staggered entry */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 10 }}>
        {Array.from({ length: 6 }).map((_, i) => (<KpiCardSkeleton key={i} darkMode={darkMode} index={i} />))}
      </div>
        ...
        <SectionCardSkeleton height={220} darkMode={darkMode} delay={480}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", gap: 14, marginBottom: 4 }}>
              {Array.from({ length: 4 }).map((_, i) => (<SkeletonBox key={i} width={80} height={10} radius={4} darkMode={darkMode} />))}
            </div>
            <SkeletonBox width="100%" height={200} radius={8} darkMode={darkMode} />
          </div>
        </SectionCardSkeleton>
        <SectionCardSkeleton darkMode={darkMode} delay={560}>
          <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
            <SkeletonBox width={160} height={160} radius={999} darkMode={darkMode} />
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  <SkeletonBox width="90%" height={10} radius={4} darkMode={darkMode} />
                  <SkeletonBox width="100%" height={5} radius={99} darkMode={darkMode} />
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} style={{
              ...
              {Array.from({ length: 6 }).map((_, j) => (<SkeletonBox key={j} width="100%" height={14} radius={4} darkMode={darkMode} />))}
            </div>
          ))}
        </div>
      </SectionCardSkeleton>
...
// ── Main AnalyticsView ────────────────────────────────────────
export default function AnalyticsView({ ui, darkMode, chartYear, setChartYear, chartMonth, setChartMonth, rxFilter, setRxFilter, onSliceClick }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [years, sum, trend, classification, yearSum, drugs, countries, docType] = await Promise.all([
        getAnalyticsAvailableYears(), getAnalyticsSummary(params), getAnalyticsTrend(params),
        getAnalyticsByClassification(params), getAnalyticsYearSummary(), getAnalyticsTopDrugs(params),
        getAnalyticsTopCountries(countryParams), getDocTypeReleased(),
      ]);
      setAvailableYears(years.years ?? ["All"]);
      setSummary(sum); setTrendData(trend.data ?? []); setClassificationData(classification.data ?? []);
      setYearSummaryData(yearSum.data ?? []); setTopDrugsData(drugs.data ?? []);
      setTopCountriesData(countries.data ?? []); setDocTypeData(docType ?? { doc_types: [], data: [] });
  ...
  const pieData = [{ name: "CPR Released", value: summary.cpr }, { name: "LOD Released", value: summary.lod }, { name: "On Process", value: summary.on_process }];
  ...
  const neuBtnStyle = (isActive) => ({
    ...
    cursor: "pointer",
    fontFamily: font,
    boxShadow: isActive ? neuShadow(darkMode, false) : neuShadow(darkMode, true),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, fontFamily: font, background: neuBg(darkMode), padding: 16, borderRadius: 20 }}>

      {/* ── Header ── */}
      <FadeSlideIn delay={0}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: "1.1rem", fontWeight: 700, color: ui.textPrimary }}>📊 Analytics Overview</h2>
            <p style={{ margin: "3px 0 0", fontSize: "0.75rem", color: ui.textMuted }}>
              Real-time data from main database{lastUpdated && <span> · Updated {lastUpdated.toLocaleTimeString("en-PH", { hour: "2-digit", minute: "2-digit" })}</span>}
            </p>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", background: neuCardBg(darkMode), borderRadius: 12, padding: 4, gap: 3, boxShadow: neuShadow(darkMode, false) }}>
                {[{ key: "All", label: "All" }, { key: "Over-the-Counter (OTC)", label: "OTC" }, { key: "Vaccine", label: "Vaccine" }, { key: "Prescription Drug (RX)", label: "RX" }].map(({ key, label }) => (
                  <button key={key} onClick={() => setRxFilter(key)} style={neuBtnStyle(rxFilter === key)}>{label}</button>
                ))}
              </div>
            </div>
            {/* Year */}
            ...
            <div>
              ...
              <select value={chartMonth} onChange={(e) => setChartMonth(e.target.value)} disabled={chartYear === "All"} style={{ ...inputSt, minWidth: 110, opacity: chartYear === "All" ? 0.4 : 1 }}>
                <option value="All">All Months</option>
                {MONTHS.map((m, i) => <option key={i} value={i}>{m}</option>)}
              </select>
            </div>
            {/* Refresh */}
            <button onClick={fetchAll} title="Refresh" style={{ ...inputSt, padding: "7px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, boxShadow: neuShadow(darkMode) }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 10 }}>
        {[
          ...
          { icon: "🏁", label: "Completed", value: summary.completed, color: "#0891b2", sub: "With other application categories" },
          { icon: "📈", label: "Approval %", value: `${summary.approval_rate}%`, color: "#9333ea", sub: "CPR over total" },
        ].map((kpi, i) => (<KpiCard key={kpi.label} {...kpi} darkMode={darkMode} ui={ui} animDelay={i * 60} />))}
      </div>

      {/* ── Trend + Donut ── */}
      <FadeSlideIn delay={420}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 14 }}>
          <SectionCard title="Trend Overview" subtitle={`Grouped by ${chartYear === "All" ? "year" : "month"}`} icon="📈" ui={ui} darkMode={darkMode}>
            <TrendChart data={trendData} ui={ui} darkMode={darkMode} />
          </SectionCard>
          <SectionCard title="Approval Breakdown" subtitle="CPR vs LOD vs On Process" icon="🍩" ui={ui} darkMode={darkMode}>
      ...
      <FadeSlideIn delay={500}>
        <SectionCard title="Year-by-Year Summary" icon="📅" ui={ui} darkMode={darkMode}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem" }}>
                <thead>
                  <tr style={{ background: darkMode ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)" }}>
                    {["Year", "Total"].map(h => (<th key={h} style={{ padding: "8px 12px", textAlign: h === "Year" ? "left" : "center", fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: ui.textMuted, borderBottom: `1px solid ${darkMode ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)"}`, whiteSpace: "nowrap" }}>{h}</th>))}
                    {docTypeData.doc_types.map(dt => (<th key={dt} style={{ padding: "8px 12px", textAlign: "center", fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: ui.textMuted, borderBottom: `1px solid ${darkMode ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)"}`, whiteSpace: "nowrap" }}>{dt}</th>))}
                    <th style={{ padding: "8px 12px", textAlign: "center", fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: ui.textMuted, borderBottom: `1px solid ${darkMode ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)"}`, whiteSpace: "nowrap" }}>Rate</th>
                  </tr>
                </thead>
                <tbody>
                  {yearSummaryData.map((row, i) => {
                    const docRow = docTypeData.data.find(d => String(d.year) === String(row.year));
                    return (
                      <tr key={row.year} style={{ borderBottom: i < yearSummaryData.length - 1 ? `1px solid ${darkMode ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)"}` : "none" }}
                        ...
                        {docTypeData.doc_types.map(dt => {
                          ...
                          const color = dt === "CPR" ? "#36a420" : dt === "LOD" ? "#e02020" : dt === "ON PROCESS" ? "#f59e0b" : ui.textPrimary;
                          return (<td key={dt} style={{ padding: "9px 12px", textAlign: "center", fontWeight: 600, color }}>{count}</td>);
                        })}
      ...
      <FadeSlideIn delay={560}>
        <SectionCard title="By Classification" subtitle="Prescription type breakdown" icon="💊" ui={ui} darkMode={darkMode}>
          {classificationData.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
              {classificationData.map((item, idx) => {
                ...
                return (
                  <div key={item.type} style={{ padding: 12, borderRadius: 12, background: neuCardBg(darkMode), boxShadow: neuShadow(darkMode) }}>
                    ...
                    <MiniBar value={parseFloat(pct)} max={100} color={color} darkMode={darkMode} />
      ...
      <FadeSlideIn delay={620}>
        <SectionCard title="Top Drug Applications" subtitle="Ranked by application volume" icon="💊" ui={ui} darkMode={darkMode}
          ...
          {topDrugsData.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {topDrugsData.map((drug, i) => {
                ...
                return (
                  <div key={drug.name} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 80px 80px 80px", gap: 8, padding: "10px 4px", borderRadius: 10, borderBottom: i < topDrugsData.length - 1 ? `1px solid ${darkMode ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)"}` : "none", alignItems: "center" }}
                    onMouseEnter={e => e.currentTarget.style.background = darkMode ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)"}
                    onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <MiniBar value={drug.total} max={maxDrug} color={FB} darkMode={darkMode} />
      ...
      <FadeSlideIn delay={680}>
        <SectionCard title="Top Countries by Entity Type" subtitle="Country of origin per application role" icon="🌍" ui={ui} darkMode={darkMode}
          action={
            ...
            {ENTITY_TYPES.map(et => {
              ...
              return (
                <button key={et.key} onClick={() => setTopCountryTab(et.key)} style={{
                  ...
                  boxShadow: isAct ? neuShadow(darkMode, false) : neuShadow(darkMode, true),
                  transition: "all 0.2s",
                }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {topCountriesData.map((d, i) => {
                  ...
                  return (
                    <div key={d.country} style={{ padding: "8px 10px", borderRadius: 10, background: neuCardBg(darkMode), boxShadow: neuShadow(darkMode) }}>
                      ...
                      <MiniBar value={d.count} max={maxCountry} color={activeEntity?.color || FB} darkMode={darkMode} />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ background: neuCardBg(darkMode), borderRadius: 12, padding: 14, boxShadow: neuShadow(darkMode, false) }}>
                    ...
                    {[1, 0, 2].map(rank => {
                      ...
                      return (
                        <div key={rank} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                          ...
                          <div style={{ width: "100%", background: neuCardBg(darkMode), borderRadius: "10px 10px 0 0", height: heights[rank], display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2, boxShadow: neuShadow(darkMode) }}>
                          ...
                          <div style={{ width: "100%", background: neuCardBg(darkMode), borderRadius: "0 0 8px 8px", padding: "4px 0", textAlign: "center", boxShadow: neuShadow(darkMode, false) }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {[
                    ...
                    <div key={s.label} style={{ padding: "10px 12px", borderRadius: 10, background: neuCardBg(darkMode), boxShadow: neuShadow(darkMode) }}>
