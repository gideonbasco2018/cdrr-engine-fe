...
export const generatePDF = async (selectedData, activeTab) => {
  ...
  doc.autoTable({
    ...
    columnStyles: {
      _no:             { halign: "center", cellWidth: 10,  valign: "middle" },
      ...
      ltoCompany:      { cellWidth: 31,    valign: "middle" },
      ltoAdd:          { cellWidth: 35,    valign: "middle" },
...
/* ================================================================== */
/*  GMP TRANSMITTAL — PDF                                              */
/*  Fields: barcode/DTN, Name of Establishment, Issuance Type,         */
/*  SECPA Number, Certificate Number.                                  */
/* ================================================================== */
export const generateGMPTransmittalPDF = async (selectedData, activeTab) => {
  await loadScript(
    "https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js",
  );
  await loadScript(
  ...
  /* ── Columns ── */
  // Barcode and DTN are separate columns (not stacked in one cell), and
  // widths are sized to add up to the full usable page width (pageW minus
  // the 10mm left/right margins) instead of leaving blank space on the right.
  const cols = [
    { header: "#",                    dataKey: "_no"       },
    { header: "Barcode",              dataKey: "_barcode"  },
    { header: "DTN",                  dataKey: "dtn"        },
    { header: "Name of Establishment",dataKey: "estName"    },
    { header: "Issuance Type",        dataKey: "issuance"   },
    { header: "SECPA Number",         dataKey: "secpa"      },
    { header: "Certificate Number",   dataKey: "certNo"     },
  ];
  ...
  const rows = selectedData.map((r, i) => ({
    ...
    _barcode: "",
    dtn:      clean(r.dtn),
    estName:  clean(r.name_of_establishment),
    issuance: clean(r.type_of_issuance),
    secpa:    clean(r.secpa_number),
    certNo:   clean(r.certificate_number),
  }));
  ...
  doc.autoTable({
    ...
    columnStyles: {
      _no:      { halign: "center", cellWidth: 10, valign: "middle" },
      _barcode: { cellWidth: 30,    halign: "center", valign: "middle" },
      dtn:      { cellWidth: 30,    halign: "center", valign: "middle", fontStyle: "bold" },
      estName:  { cellWidth: 75,    valign: "middle" },
      issuance: { cellWidth: 50,    valign: "middle" },
      secpa:    { cellWidth: 41,    halign: "center", valign: "middle" },
      certNo:   { cellWidth: 41,    halign: "center", valign: "middle" },
    },
    didDrawCell: (h) => {
      if (h.section === "body" && h.column.dataKey === "_barcode") {
        const img = barcodeImages[h.row.index];
...
/* ================================================================== */
/*  GMP TRANSMITTAL — EXCEL                                            */
/* ================================================================== */
export const generateGMPTransmittalExcel = async (selectedData, activeTab) => {
  await loadScript(
    "https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js",
  );
  ...
  selectedData.forEach((r, i) => {
    wsData.push([
      i + 1,
      clean(r.dtn),
      clean(r.name_of_establishment),
      clean(r.type_of_issuance),
      clean(r.secpa_number),
      clean(r.certificate_number),
    ]);
  });

  const ws = window.XLSX.utils.aoa_to_sheet(wsData);
  ws["!cols"] = [4, 20, 40, 26, 20, 24].map((wch) => ({ wch }));
...
/* ================================================================== */
/*  GMP TRANSMITTAL — COMBINED                                         */
/* ================================================================== */
export const generateGMPTransmittal = async (selectedData, activeTab) => {
