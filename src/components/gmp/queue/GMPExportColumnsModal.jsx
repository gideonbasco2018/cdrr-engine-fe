// FILE: src/components/gmp/queue/GMPExportColumnsModal.jsx
// FGMP counterpart of components/reports/ExportColumnsModal.jsx (CPR) — same
// "Select columns to include" flow, pointed at the GMP export-columns list.
import { useState, useEffect, useMemo } from "react";
import { getGMPExportColumns } from "../../../api/gmp";

export default function GMPExportColumnsModal({ onClose, onConfirm, colors, darkMode }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const cols = await getGMPExportColumns();
        setColumns(cols);
        setSelected(new Set(cols.map((c) => c.id))); // all checked by default
      } catch (err) {
        console.error("Failed to fetch GMP export columns:", err);
    ...
    >
      <div
        style={{
          ...
          borderRadius: 14, padding: "1.5rem", width: 520, maxWidth: "90vw",
      ...
      >
        ...
        <div style={{ overflowY: "auto", flex: 1, paddingRight: "0.25rem" }}>
          {loading && (
            <p style={{ fontSize: "0.8rem", color: colors.textSecondary }}>Loading columns…</p>
          )}
          {error && <p style={{ fontSize: "0.8rem", color: "#ef4444" }}>{error}</p>}
          {!loading && !error && Object.entries(grouped).map(([group, cols]) => {
              ...
              <div key={group} style={{ marginBottom: "0.75rem" }}>
                <label
                  style={{
                    ...
                    fontSize: "0.72rem", color: colors.textTertiary, textTransform: "uppercase",
                    marginBottom: 4, cursor: "pointer",
                  }}
                >
                  <input type="checkbox" checked={allChecked} onChange={(e) => toggleGroup(cols, e.target.checked)} />
                  {group} ({cols.length})
                </label>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, paddingTop: "0.5rem", borderTop: `1px solid ${colors.cardBorder}` }}>
          <button
            onClick={onClose}
            style={{
              padding: "6px 14px", borderRadius: 6, border: `1px solid ${colors.cardBorder}`,
              background: "transparent", color: colors.textPrimary, cursor: "pointer", fontSize: "0.78rem",
            }}
          >
            Cancel
          </button>
